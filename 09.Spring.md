...
<img src="images/198.png" alt="198" style="zoom:67%;" />
...
AOP (Aspect Oriented Programming) 意为：面向切面编程，通过预编译的方式和运行期动态代理实现程序功能的统一维护的一种技术。AOP是OOP的延续，是软件开发中的一个热点，也是Spring框架中一个重要内容，是函数式编程的一种衍生范型。利用AOP可以对业务逻辑的各个部分进行隔离，从而使得业务逻辑各部分之间的耦合度降低，提高程序的可重用性，同时提高了开发的效率。

<img src="images/199.png" alt="199" style="zoom:60%;" />
...
<bean id="log" class="com.jin.log.Log"/>
...
<aop:config>
    <!--切入点: expression: 表达式, execution: 要执行的位置
        execution(修饰符【可省略，默认public】，返回值【*代表任意】，
        包名.类名/接口名.方法名(参数列表)【(..)所有参数，(*)一个参数，
		(*,String)第一个参数任意值，第二个String类型】)-->
    <aop:pointcut id="pointcut" expression="execution(* com.jin.service.UserServiceImpl.*(..))"/>
...
```xml
<!--方式二：自定义类-->
<bean id="diy" class="com.jin.diy.DiyPointCut"/>

<aop:config>
    <!--自定义切面，ref要引用的类-->
    <aop:aspect ref="diy">
        <!--切入点-->
        <aop:pointcut id="point" expression="execution(* com.jin.service.UserServiceImpl.*(..))"/>
        <!--通知-->
...
```xml
<!--方式三-->
<bean id="annotationPointCut" class="com.jin.diy.AnnotationPointCut"/>
...
2. 编写核心配置文件
   ...
   <configuration>
       ...
       <mappers>
           <mapper class="com.jin.mapper.UserMapper"/>
       </mappers>
   </configuration>
   ```
...
* 可能问题：UserMapper.xml无法被导出或者生效的问题(out文件夹下xml文件不生成，由于没有放在resourses文件夹下，因此不会保留) pom.xml下添加如下代码[7.JavaWeb/Maven部分有提及(重点!)](7.JavaWeb.md/#ConvOverConf)
...
2. sqlSessionFactory（配置后utils类中的工具类不再需要）

3. sqlSessionTemplate

   Spring-dao.xml实现上面三者
   ...
   <beans xmlns="http://www.springframework.org/schema/beans"
       ...
       <bean id="sqlSessionFactory" class="org.mybatis.spring.SqlSessionFactoryBean">
           ...
           <property name="mapperLocations" value="classpath:com/jin/mapper/*.xml"/>
       </bean>
...
4. 需要给接口加实现类【新的】
   ...
   原因：spring接管类会自动创建实现类，mybatis不行，所以这里要自己加实现类
...
5. 将自己写的实现类，注入到spring中
   ...
   这里新建一个applicationContext,xml作为主要配置文件，并将spring-dao导入，在这里注入实现类

   ```xml
   <import resource="spring-dao.xml"/>
   
   <!--实现类-->
   <bean id="userMapper" class="com.jin.mapper.UserMapperImpl">
...
2. sqlSessionFactory （1，2两步同上）

3. 给接口加实现类继承SqlSessionDaoSupport（相当于将上面3，4合并）
...
4. 将自己写的实现类，注入到spring中
   ...
   <!--实现类-->
   <bean id="userMapper2" class="com.jin.mapper.UserMapperImpl2">
...
1. 创建addUser, deleteUser 方法，将其加入selectUser中
...
出现错误，观察数据库，发现数据库多了一行数据，说明事务原则没有保证

### 2. Spring中的事务管理

* 声明式事务：AOP
* 编程式事务：需要在代码中进行事务的管理 (直接在java上添加TransactionStatus 和try catch语句确保回滚，不推荐)
...
1. spring-dao.xml
   ...
   <!--配置事务切入-->
   <aop:config>
       <aop:pointcut id="txPointCut" expression="execution(* com.jin.mapper.*.*(..))"/>
